...
function renderCategoryFilter(allCategories, props) {
    ...
    return (
      <li key={i} className="c-checkbox-filter__item">
        <label htmlFor={cat.slug}>
          ...
          />
          <span className="c-checkbox-filter__icon" style={{ backgroundColor: cat.color }}>
            <i className={classNames('fa', cat.icon)} aria-hidden="true"></i>
          </span>
          {cat.name}
        </label>
      </li>
  ...
  return (
    <ul className="c-checkbox-filter categories-filter__checkboxes">
